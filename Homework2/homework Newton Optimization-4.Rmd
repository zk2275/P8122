...
LASSO = function(x, y, lambda, tol = 0.01, warm_start = FALSE, include_zero_lambda = TRUE){
  ...
  # Multiple lambda case: sort lambda in decreasing order and iterate over it
    ...
    for (k in 1:length(lambda)) {
      if (warm_start)
        beta = Pathwise(x, y, lambda[k], beta, tol)$beta_temp # Use previous β as starting point if warm_start is TRUE
      else{
        beta = matrix(rep(0, ncol(x))) # Reset β to zeros if not using warm_start
        beta = Pathwise(x, y, lambda[k], beta, tol)$beta_temp
      }
      beta_list[[paste("beta <- lambda:", lambda[k])]] = beta # Store β vector for current λ
    }
    return(list(lambda = lambda, beta = beta_list, colmean = colmean, colscale = colscale)) # Return λ sequence, β vectors, column means and scales
  }
}
